...
namespace ATM.Services
{
    public class Startup
    {
        internal static void Initiate()
        {
            ...
            string table_name = "BankData";
            sqlite.DeleteData(sqlite.Conn, table_name);
            if (sqlite.CheckIfTableEmpty(sqlite.Conn, table_name)==true)
            {
                for (int i = 0; i < 10; i++)
                {
                ...
                var balance = Math.Round((Math.PI * (i+1 * 2000) - (i+1)*500),2);
                var blocked = false;
